    ...
    def build_budget_task(
        ...
        *,
        ...
        task = TaskContract(
            ...
            idempotency_key=f"{campaign_id}:update_budget:{amount}",
            payload={
                "campaign_id": campaign_id,
                "budget": {"amount": round(amount, 2), "currency": "USD", "type": "daily"},
            },
        ...
        task["payload"]["budget"] = {"amount": round(amount, 2), "currency": "USD", "type": "daily"}
        if allocation:
...
class ApprovalRouter:
    def route(self, task: dict[str, Any], *, risk_category: str, sensitive_vertical: bool, requested_by: str) -> str | None:
        ...
        if delta_pct > 30 or daily_cap > task["safety_profile"].get("critical_daily_budget_threshold", 1_000_000):
            return "dual"
...
class PolicyGate:
    ...
    def evaluate(self, decision: Decision, context: Any) -> Decision:
        ...
        if decision.rule_name == "creative_fatigue_rotation" and context.creative_rotation_count_today >= self.max_creative_rotations_per_day:
            return Decision(decision.rule_name, decision.reason, None, blocked_reason="creative_rotation_cap_reached")
