...
from affiliate_agent.policies import ApprovalRouter, Decision, PolicyGate, TaskGenerator
...
class DecisionEngine:
        ...
        if self._should_pause_for_refunds(context):
            return Decision(
                ...
                self.generator.build_pause_task(
                    ...
                    critical_daily_budget_threshold=context.critical_daily_budget_threshold,
                ),
            )
        ...
        if self._should_rotate_creative(context):
            return Decision(
                ...
                self.generator.build_rotation_task(
                    ...
                    next_creative_id=context.creative_candidates[0],
                    daily_spend_cap=context.daily_spend_cap,
                    critical_daily_budget_threshold=context.critical_daily_budget_threshold,
                ),
            )
    ...
    def _should_pause_for_refunds(self, context: DecisionContext) -> bool:
        return (
            ...
            and (
                context.current_refund_rate >= context.refunds_baseline_rate * 2
                or context.complaints_anomaly
            )
        )
